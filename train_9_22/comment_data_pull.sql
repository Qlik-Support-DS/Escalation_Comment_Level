...
escalate_aggregate_to_comment as (
...
//    aac.has_escalated_val
  ...
  from case_comments cc left join account_and_case_analyze aac
    on cc.parent_id = aac.id
    and cc.created_at > aac.closed_at //closed_at
  qualify n_seq = 1
),

account_and_case_analyze_aggregate_to_comment_created_at as (
select cc.id, cc.parent_id, cc.created_at, 
    Row_number() over (partition by cc.id order by aac.created_at desc) as n_seq,
    aac.n_escalated,
    aac.n_case,
...
select cc.id,
        ...
        aac.avg_priority,
        aac.avg_severity,
        ea.has_escalated_val,
        ea.account_id,
        aja.bug_weight_c,
        aja.aja_created_at as associated_jira_aggregate_created_at,
...
from case_comments cc
    ...
    left join escalate_aggregate_to_comment ea on cc.id = ea.id
...
//select bug_weight_c, bug_weight_avg_up_to_comment, associated_jira_aggregate_created_at, n_jira_tickets_up_to_comment, created_at
//from output
//where parent_id = '5003z00002gl0LpAAI'
//order by created_at
